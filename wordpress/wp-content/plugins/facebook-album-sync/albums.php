...
var fblink = data.data[i].link;
var fbid_start = fblink.indexOf('?');
var id_start = fblink.indexOf('&', fbid_start);
var aid_start = fblink.indexOf('&', id_start); 
var fbid = fblink.substring(fbid_start+1, id_start);
var id = fblink.substring(id_start, aid_start);
var aid = fblink.substring(aid_start);
										if(prettypermalinkon==1){
											//photoslink = document.URL+"?fbasid="+data.data[i].id;
                                                                                         photoslink = '?page_id=90&'+fbid + id + aid;
										}else{
											//photoslink = document.URL+"&fbasid="+data.data[i].id;
                                                                                        photoslink = '?page_id=90&'+fbid + id + aid;
										}
										...
										if (i < data.data.length - 1){
											...
											getAlbums(data, i );
console.log('processing album >> '+i+' album title >> '+albumname);
										}else
										{
